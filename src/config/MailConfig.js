...
async function send (sendInfo) {
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      user: '719791415@qq.com', // generated ethereal user
      pass: 'opmijibgggmjbdbh' // generated ethereal password
    }
  })
  // const baseUrl = config.baseUrl
  // const route = sendInfo.type === 'email' ? '/confirm' : '/reset'
  const url = 'https://www.imooc.com'
  // `${baseUrl}/#${route}?` + qs.stringify(sendInfo.data)
  // send mail with defined transport object
  const mailOptions = {
    from: '"认证邮件" <719791415@qq.com>', // sender address
